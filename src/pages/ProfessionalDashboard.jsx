...
export default function ProfessionalDashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [profileRes, empresaRes, configRes, budgetRes] = await Promise.all([
      ...
      setPresupuestos(budgetRes.data ?? [])
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  async function handleSaveEmpresa() {
    ...
    const { error } = await supabase
      ...
      .upsert({ ...empresaEdit, user_id: user.id })
  ...
  const fmtDate = (d) => new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(d))

  const totalFacturado = presupuestos.reduce((a, b) => a + (b.total_with_iva ?? 0), 0)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              <img src="/persianassantanderlogo.svg" alt="Persianas Santander" className="w-20 h-auto"
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              {TABS.map(({ id, label, icon }) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors text-left border-b border-gray-100 last:border-0 ${
                    activeTab === id
                      ? 'bg-red-50 text-red-700 border-l-2 border-l-red-700'
          ...
          <main className="flex-1 min-w-0">
            ...
            {activeTab === 'presupuestos' && (
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-gray-900">Presupuestos y pedidos</h2>
                {presupuestos.length === 0 ? (
                  <EmptyState label="No tienes presupuestos todavía" action="Solicitar presupuesto" to="/configurador" />
                ) : (
                  <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b border-gray-200">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {presupuestos.map(p => (
                          <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-5 py-3 font-mono text-xs text-gray-600">{p.budget_number}</td>
            ...
            {activeTab === 'facturas' && (
              <div className="space-y-4">
                ...
                {presupuestos.filter(p => p.status === 'completed').length === 0 ? (
                  <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                    ...
                    <p className="text-gray-400 text-sm">Las facturas aparecerán aquí cuando se completen tus pedidos.</p>
                  </div>
                ) : (
                  <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                          <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Factura</th>
                          <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Fecha</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {presupuestos.filter(p => p.status === 'completed').map(p => (
                          <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-5 py-3 font-mono text-xs text-gray-600">{p.budget_number}</td>
                            <td className="px-5 py-3 text-gray-400 hidden md:table-cell">{fmtDate(p.created_at)}</td>
                            <td className="px-5 py-3 text-right font-bold text-gray-900">{fmt(p.total_with_iva)}</td>
                            <td className="px-5 py-3 text-right">
                              {p.excel_url
                                ? <a href={p.excel_url} target="_blank" rel="noreferrer"
                                    className="text-red-700 hover:underline text-xs font-semibold">Descargar</a>
            ...
            {activeTab === 'empresa' && (
              <div className="space-y-5">
                ...
                <div className="bg-gray-50 rounded-xl border border-gray-200 p-5">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Información de cuenta</p>
                  <div className="flex items-center gap-3">
