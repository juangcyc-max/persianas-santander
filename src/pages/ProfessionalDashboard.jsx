          ...
          <main className="flex-1 min-w-0">
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                {/* Datos de empresa — resumen rápido */}
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-gray-700">Datos de empresa</span>
                    </div>
                    <button onClick={() => setActiveTab('empresa')}
                      className="text-xs font-semibold text-red-700 hover:text-red-800 transition-colors">
                      {empresaCompleta(empresa) ? 'Editar' : 'Completar datos →'}
                    </button>
                  </div>
                  {empresaCompleta(empresa) ? (
                    <div className="px-5 pb-4 grid grid-cols-2 gap-x-6 gap-y-1 text-sm border-t border-gray-100 pt-3">
                      <div><span className="text-gray-400 text-xs">Razón social</span><p className="text-gray-800 font-medium">{empresa.razon_social}</p></div>
                      <div><span className="text-gray-400 text-xs">CIF/NIF</span><p className="text-gray-800 font-medium">{empresa.cif_nif}</p></div>
                      <div><span className="text-gray-400 text-xs">Dirección fiscal</span><p className="text-gray-800">{empresa.direccion_fiscal}</p></div>
                      <div><span className="text-gray-400 text-xs">CP / Ciudad</span><p className="text-gray-800">{[empresa.codigo_postal, empresa.ciudad].filter(Boolean).join(' ')}</p></div>
                      <div><span className="text-gray-400 text-xs">Teléfono</span><p className="text-gray-800">{empresa.telefono}</p></div>
                      <div><span className="text-gray-400 text-xs">Email facturación</span><p className="text-gray-800 truncate">{empresa.email_facturacion}</p></div>
                    </div>
                  ) : (
                    <p className="px-5 pb-4 text-sm text-amber-600 border-t border-gray-100 pt-3">
                      Completa los datos de tu empresa para que aparezcan en facturas y presupuestos.
                    </p>
                  )}
                </div>
